...
const MainCategories = () => {
  ...
  return (
    <div className="rounded-2xl border border-zinc-200/80 bg-zinc-50/70 p-2.5 sm:p-3 backdrop-blur-sm">
      {/* Category Pills */}
        ...
        {categoryLinks.map(({ to, label, cat }) => {
          ...
          return (
            <Link
              key={label}
              href={to}
              className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 sm:px-3.5 sm:py-2 text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-200 ${
                isActive
