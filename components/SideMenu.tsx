  ...
  return (
    <aside className="sticky top-24 space-y-6">
      ...
      <div className="rounded-3xl border border-zinc-200/80 bg-white p-5 shadow-xs">
        <h3 className="mb-3 text-xs font-bold uppercase tracking-wider text-zinc-400">Rechercher</h3>
        <Search />
      </div>

      {/* Sort Filters Card */}
      <div className="rounded-3xl border border-zinc-200/80 bg-white p-5 shadow-xs">
        <h3 className="mb-3 text-xs font-bold uppercase tracking-wider text-zinc-400">Trier par</h3>
      ...
      <div className="rounded-3xl border border-zinc-200/80 bg-white p-5 shadow-xs">
        ...
        <div className="flex flex-wrap gap-2">
          {categoryOptions.map((cat) => {
            const isSelected = currentCat === cat.value || (cat.value === "general" && !searchParams.get("cat"));
            return (
              <button
                key={cat.value}
                type="button"
